...
class PantallaDialogs extends StatelessWidget {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(esError
            ? 'Error: no se pudo conectar al muelle'
  ...
  Future<void> _mostrarConfirmacion(BuildContext context) async {
    final confirmar = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title:   const Text('Eliminar muelle'),
        content: const Text(
    ...
    if (confirmar == true) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Muelle eliminado correctamente')),
      );
    }
  }
  ...
  Future<void> _mostrarFormulario(BuildContext context) async {
    ...
    await showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Agregar muelle'),
        content: Form(
          key: formKey,
          child: Column(
    ...
    if (ctrlNombre.text.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Muelle "${ctrlNombre.text}" agregado')),
      );
    }
  }
      ...
      SnackBar(
        content: Row(
          children: [
            ...
            Expanded(
              child: Text(
                'Nota: El muelle web-02 está programado para mantenimiento.',
  ...
  Future<void> _mostrarDialogoDetallado(BuildContext context) async {
    final cs = Theme.of(context).colorScheme;
    await showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        icon: Icon(Icons.verified_user, size: 40, color: cs.tertiary),
        title: const Text('Autenticación de Llave SSH'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text(
              'Se requiere una huella digital para validar el acceso:',
            ...
            Container(
              ...
              child: const Text(
                'SHA256:q9n8J38hD98jH12oPl90KlJu8hsa7shA',
                style: TextStyle(fontFamily: 'monospace', fontSize: 12),
              ),
            ),
            const SizedBox(height: 12),
            Text(
              '¿Deseas guardar esta clave permanentemente?',
      ...
      body: ListView(
        ...
        children: [
          ...
          FilledButton.icon(
            ...
            onPressed: () => _mostrarSnackBarPersonalizado(context),
          ...
          OutlinedButton.icon(
            ...
            label: const Text('Eliminar muelle (confirmación)'),
          ),
          const SizedBox(height: 8),
          FilledButton.tonal(
            onPressed: () => _mostrarFormulario(context),
            child: const Text('Agregar muelle (formulario)'),
          ),
          const SizedBox(height: 8),
          FilledButton.icon(
            style: FilledButton.styleFrom(
            ...
            onPressed: () => _mostrarDialogoDetallado(context),
            icon:  const Icon(Icons.verified_user),
            label: const Text('Dialog Personalizado'),
          ),
        ],
      ),
    );
