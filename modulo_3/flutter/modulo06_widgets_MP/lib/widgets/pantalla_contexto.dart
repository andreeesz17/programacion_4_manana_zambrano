  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          'Terminal ${esMovil ? "móvil" : "central"} · Operador ${esRetrato ? "en muelle" : "en grúa"}',
      ...
      body: ListView(
        ...
        children: [
          ...
          _Seccion(
            titulo: 'Especificaciones de Buque en Tránsito',
            items: [
              'Eslora (m):        ${tamanio.width.toStringAsFixed(0)}',
              'Manga (m):         ${tamanio.height.toStringAsFixed(0)}',
              'Calado (m):  ${MediaQuery.devicePixelRatioOf(context).toStringAsFixed(3)}',
              'Capacidad TEU:  ${MediaQuery.paddingOf(context).top}',
            ],
          ),
          const SizedBox(height: 16),

          // ── Colores del tema ───────────────────────────────────
          _Seccion(titulo: 'Niveles de Alerta del Puerto', items: []),
          Wrap(
            spacing: 8, runSpacing: 8,
            children: [
              _ChipColor(nombre: 'normal',          color: colores.primary),
              _ChipColor(nombre: 'primaryContainer', color: colores.primaryContainer),
              _ChipColor(nombre: 'secondary',        color: colores.secondary),
              _ChipColor(nombre: 'manifiestoOK',          color: colores.surface),
          ...
          // ── Tipografía del tema ────────────────────────────────
          _Seccion(titulo: 'Tipografías del Manifiesto Aduanero', items: []),
          Text('Título de Contenedor',  style: tema.textTheme.displaySmall),
          Text('Subtítulo de Carga', style: tema.textTheme.headlineMedium),
          Text('Nombre de Remolcador',    style: tema.textTheme.titleLarge),
          Text('Detalle de Inspección',     style: tema.textTheme.bodyLarge),
          Text('Notas de Aduana',    style: tema.textTheme.bodyMedium),
          Text('Identificador TEU',    style: tema.textTheme.labelSmall),
        ],
      ),
    );
  }
